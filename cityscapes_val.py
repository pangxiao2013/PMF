...
def main():
    ...
    class_mean_epp = np.load(f'stats/{args.dataset[0]}_mean.npy', allow_pickle=True)
    class_var_epp = np.load(f'stats/{args.dataset[0]}_var.npy', allow_pickle=True)
...
def validate(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter, save_pth=True):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        predictions = main_out.max(1)[1]

        anomaly_score = -anomaly_score
        ones = torch.ones(anomaly_score.shape[0], anomaly_score.shape[1],
                          anomaly_score.shape[2],dtype=torch.long) * 0
        ones = ones.cuda()
        temp = predictions
        predictions = torch.where(anomaly_score > -0.05244519710540771, #0.34, #0.1929
                                  ones,predictions)
