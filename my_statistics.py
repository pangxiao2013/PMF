...
default='/home/dl/PycharmProjects/Standardized-max-logits/datasets/Static',
                    help='OoD dataset path')

# Anomaly score mode - msp, max_logit, standardized_max_logit
parser.add_argument('--score_mode', type=str, default='PMF',
                    help='score mode for anomaly [msp, max_logit, standardized_max_logit, PMF]')
...
parser.add_argument('--boundary_width', type=int, default=9,
                    help='initial boundary suppression width')
parser.add_argument('--boundary_iteration', type=int, default=9,
    ...
    for city in cities:
        ...
        for image_file in tqdm(os.listdir(city_path)):
            ...
            with torch.no_grad():
                ...
                prediction, anomaly_score = net(image)
                prediction_temp = prediction.cpu().numpy()
                anomaly_score_temp = anomaly_score.cpu().numpy()
                prediction_temp = np.squeeze(prediction_temp, 0)
                anomaly_score_temp = np.squeeze(anomaly_score_temp, 0)
                prediction_temp = np.expand_dims(prediction_temp,2)
                anomaly_score_temp = np.expand_dims(anomaly_score_temp, 2)
                ood_gts = np.expand_dims(ood_gts, 2)
                two_combine = np.concatenate([prediction_temp,anomaly_score_temp],axis=2)
                three_combine = np.concatenate([two_combine,ood_gts],axis=2)
                if img_str in ['4','17','25','26']: #['2','3','7','11','76','83']: #['4','68','95','19']
                    np.save('/home/dl/Desktop/myn/heatmap_visual/ml/'+img_str+".npy", three_combine)
                count+=1
                # print(three_combine.shape)
                # print(three_combine[0,0])
                # exit(0)
